...
- **Zero-initialized FiLM** — feature-wise linear modulation whose learned projections start
  from gamma = 1 and beta = global context in the released implementation.
...
recorded package versions and measurement context. See `../IMPLEMENTATION_NOTES.md` for observed behavior that is preserved for exact v1.0.0 reruns.
...
| `scripts/analyze_grouping_statistics.py` | Bayesian correlated-t ROPE, Wilcoxon-Holm, and Friedman summaries regenerated from the archived grouping CSV |
| `scripts/precompute_grouping_plans.py` | Export grouping permutations for standalone inspection; benchmark entry points recompute plans and do not load the exported NPZ |
...
The archived grouping statistics can be regenerated without rerunning training:

```bash
python scripts/analyze_grouping_statistics.py --output grouping_statistics.generated.json
```
...
@misc{park2026vgit,
  ...
  year    = {2026},
  note    = {Manuscript and reproducibility package, version 1.0.0},
  url     = {https://github.com/PolnareffPark/VG-iT}
}
```
