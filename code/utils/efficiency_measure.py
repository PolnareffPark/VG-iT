...
Protocol identifier: vgit-efficiency-profile-v1.
"""
...
def pre_allocate_dummy_tensors_train(
    ...
    semantics under the public efficiency profiling protocol.
...
class EfficiencyCollector:
    ...
    def capture_train_vram_peak(self) -> None:
        if _is_cuda(self.device):
            torch.cuda.synchronize(self.device)  # Ensure pending operations are complete.
