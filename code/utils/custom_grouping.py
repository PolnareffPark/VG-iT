...
def rebalance_groups(
    ...
    Algorithm (Step 1 - oversized-group resolution):
    ...
    Step 2 (group-count restoration) is unnecessary because only moves occur.
    Step 3 (undersized groups): min_size=1 allows singletons; if min_size>1,
    ...
    # Step 1: Move variates from oversized groups to the smallest groups.
    iteration = 0
    ...
    # Step 3: Handle undersized groups when min_size > 1.
    if min_size > 1:
        changed = True
        while changed:
            changed = False
