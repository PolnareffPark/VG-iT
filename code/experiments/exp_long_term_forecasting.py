...
def _load_checkpoint(model, path):
    ...
    # Security: weights_only=True prevents arbitrary code execution via malicious pickle.
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        if measure_mode_peek == 'dummy':
            # Skip validation in dummy efficiency mode to reduce worker/RAM use.
        ...
        # In dummy mode, retain only iters_per_epoch and release train_loader
        # immediately to avoid DataLoader worker contention. test_loader is
        # recreated in test(), so it is not retained here.
        ...
        # Pre-allocate GPU tensors once and reuse every iteration → no H2D, no
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(iter_source):
                ...
                if measure_mode == 'dummy':
                    # Pre-allocated GPU tensors are already float32 on the target device.
                ...
                if self._amp_enabled():
                    with self._autocast_context():
                        ...
                        f_dim = -1 if self.args.features == 'MS' else 0
                        outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        # batch_y is already on the target device in both modes.
                    ...
                    f_dim = -1 if self.args.features == 'MS' else 0
                    outputs = outputs[:, -self.args.pred_len:, f_dim:]
                    batch_y = batch_y[:, -self.args.pred_len:, f_dim:]  # Redundant .to() omitted
                    loss = criterion(outputs, batch_y)
                    train_loss.append(loss.item())
                ...
                if (i + 1) % 100 == 0:
                    ...
                    try:
                        ...
                        if getattr(self.args, 'pooling', 'mean') == 'dynamic':
                            # Hierarchical-attention grouping path
                            ...
                            # Model-level learnable grouping path
        ...
        if self._eff_collector is not None:
            ...
            # Release pre-allocated dummy tensors so test()'s VRAM measurement
    ...
    def profile_actual_flops(self):
        """
        Measure actual FLOPs with the PyTorch profiler.
        Keep this separate from training to avoid profiler overhead.
        """
        try:
    ...
    def test(self, setting, test=0):
        ...
        # Debug custom group-size propagation.
        ...
        # Dummy mode reuses pre-allocated GPU tensors every iteration.
        if self._eff_collector is not None and self._eff_collector.mode == 'dummy':
            _t_iters = len(test_loader)
            # Free DataLoader workers and pinned memory before pre-allocation so
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(_warmup_source):
                if i >= 5: break
                if self._eff_collector is not None and self._eff_collector.mode == 'dummy':
                    # Tensors are already pre-allocated on GPU and dec_inp is precomputed.
        ...
        if is_cuda:
            ...
            # Prevent training fragmentation from affecting inference peak memory.
            if self._eff_collector is not None:
                torch.cuda.empty_cache()
            torch.cuda.reset_peak_memory_stats(self.device)
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(_test_source):
                if self._eff_collector is not None and self._eff_collector.mode == 'dummy':
                    # GPU tensors are pre-allocated and dec_inp is precomputed; no H2D copy.
                    dec_inp = _v6_t_dec_inp
                else:
                    batch_x = batch_x.float().to(self.device, non_blocking=True)
                    batch_y = batch_y.float().to(self.device, non_blocking=True)
                ...
                f_dim = -1 if self.args.features == 'MS' else 0
                outputs = outputs[:, -self.args.pred_len:, f_dim:]
                # batch_y is already on device; omit redundant .to() (see train loop note).
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:]
        ...
        if self._eff_collector is not None:
            ...
            # Release pre-allocated dummy test tensors.
    ...
    def _dump_efficiency_record(self, setting, total_flops, total_params, mse, mae):
        ...
        # Train seconds per epoch (wall clock, for total runtime reporting).
        ...
        metadata = {
            ...
            'protocol': 'vgit-efficiency-profile-v1',
            'epochs_equivalent': 3,
