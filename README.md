...
This release accompanies the manuscript of the same title and is intended to make the
reported experiments auditable without redistributing third-party raw datasets or large
training checkpoints. The repository and archival metadata are intentionally independent
of any journal or submission venue.
...
| `IMPLEMENTATION_NOTES.md` | Observed v1.0.0 implementation behavior and fidelity notes for exact reruns. |
| `results_canonical/` | Curated CSV/JSON result files used by the manuscript tables, figures, and supplementary analyses. |
...
| `CITATION.cff` | Citation metadata for GitHub and archival repositories. |
...
The measured environment is recorded separately in `code/ENVIRONMENT_OBSERVED.md`. Observed historical implementation behavior that affects exact reruns is documented in `IMPLEMENTATION_NOTES.md`; the archived model and experiment logic is intentionally unchanged.
...
Result-derived statistics can be regenerated from the included CSV/JSON files with
`python code/scripts/analyze_grouping_statistics.py` and
...
This release is version `1.0.0`, published on `2026-07-29`.
