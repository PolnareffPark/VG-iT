"""
Figure 6 pre/post-pooling probing heatmaps.
Based on probing_hm_h1.py (same frame/style).

Output 1: Pre-pooling (1 panel) | Post-pooling (1 panel), aggregated over 12 conditions
Output 2: Pre (3 dataset panels, top row) | Post (3 dataset panels, bottom row), 2 x 3 grid
"""
...
OUT_FIG1 = SCRIPT_DIR / "figures" / "figure_6_pre_post_aggregate.png"
OUT_FIG2 = SCRIPT_DIR / "figures" / "figure_6_pre_post_by_dataset.png"
OUT_FIG2_PDF = SCRIPT_DIR / "figures" / "figure_6_pre_post_by_dataset.pdf"
...
def main():
    ...
    # Panel titles and the shared legend provide the figure context.
    make_legend(fig1)
    fig1.savefig(OUT_FIG1, dpi=600, bbox_inches='tight')
    plt.close(fig1)
    print(f"Saved: {OUT_FIG1}")

    # -- Per-dataset output: 2 x 3 grid (top=pre, bottom=post) --
