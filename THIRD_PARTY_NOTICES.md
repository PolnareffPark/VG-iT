...
- `code/utils/timefeatures.py` is adapted from `gluonts/src/gluonts/time_feature/_base.py`.
  ...
  - The upstream GluonTS NOTICE attribution is retained verbatim in
    `LICENSES/GLUONTS-NOTICE.txt`.
  - Upstream NOTICE snapshot observed on 2026-07-28:
    `https://github.com/awslabs/gluonts/blob/2294d8dadf7f443bc16196efb032fd64e72402d5/NOTICE`.
...
- Conservative mapped set: `code/run.py`, `code/model/iTransformer.py`,
  `code/model/iFlashformer.py`, `code/model/iNystromformer.py`,
  `code/model/VG_iTransformer.py`, `code/model/VG_iFlashformer.py`,
...
- The five listed model files share or adapt the iTransformer embedding, encoder,
  normalization, projection, and forecasting scaffold. The Flash/SDPA, Nystrom wrapper,
  and VG-iT hierarchical/grouping mechanisms are modifications or additions in this
  deposit; upstream rights remain attached to the reused scaffold.
